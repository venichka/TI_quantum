...
let
	...
	img_a = Images.load(PATH_FIGS*"fig_1a_v1.1.pdf")
	...
	# savefig(p, PATH_FIGS*"Fig_1_with_empty_v1.2.pdf")
end
...
let
	...
	# savefig(p, PATH_FIGS*"Fig_2_phipi_col_v1.1.pdf")
end
...
let
	...
	x_l = [(i ÷ length(x) < 1) ? [x[end]-i, x[end]] : [x[1], 2*x[end] - i] for i in slices]
	y_l = [(i ÷ length(x) < 1) ? [y[end], y[end]-i] : [2*y[end]-i, y[1]] for i in slices]
...
let
	...
	fig = Plots.plot(p_i[4], p_i[3], p_i[2], p_i[1],
		layout = (1, 4),
		size = (1340, 350),
		topmargin=5Plots.mm,
	...
	# Plots.savefig(fig, PATH_FIGS*"Fig_4_alpha5.0_v1.1.pdf")
...
manifest_format = "2.0"
project_hash = "06d04de981a44d9c48ebb98df36c21556bf13a22"
