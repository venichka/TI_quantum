...
Compute the second-order correlation function ``g^{(2)}(0)`` for a |n,m> initial state.
"""
...
let
	...
	# save(PATH_FIGS*"Figs_fock/Fig_3.pdf", f)
	f
end
...
let
	...
	for n in 0:100
		lines!(ax, r, map(x -> g2_n0(n, v(x)), r),
			color = :blue, 
			linewidth = 2,
			alpha=0.2,
			label=(n == 0) ? L"|n,0\rangle_0" : "",
		)
		lines!(ax, r, map(x -> g2_sym(n, v(x)), r),
			color = :red, 
			linewidth = 2,
			alpha=0.2,
...
let
	...
	for m in m_range
		idx += 1
		lines!(ax, r, map(x -> g2_nm(n, m, v(x)), r),
			color = cmap[idx], 
			linewidth = 2,
